...
With the compiler data model, driver, stage-neutral selection, and differential/reproducibility gate all at `usable/99`, deterministic coordinate ordering remains at `compiler-toolchain/bootstrap/stage0-stage1-driver`; direct stage1-owned compilation without provider delegation is now the first open boundary, followed by capability v2 and a signed generation-three forward transition, representation-sidecar aggregate binding, nested full-arena forwarding, and wider aggregate lowering without changing subset v8.
