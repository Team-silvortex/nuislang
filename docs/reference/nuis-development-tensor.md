...
and `nuis bootstrap-status --json`; migration is active while final readiness remains false at `3/5` gates.
...
Production-bound token data, two pages per AST/NIR projection, adapter-v9 cursor export, two canonical ULEB128 structural payloads without a complete source blob, `2/2` semantic replay, and generic registration-order handoff v2 selection move the stage-neutral boundary to `usable/99`.
...
Data model v10 reaches `usable/99`: it preserves every v9 identity, then registers a 24-byte `CompilerChunkedPayload` and copies it through eight fixed Nuis chunks without host collections or a new scalar export. The extended registry identity is `1593840720`; typed identity is `94500080`; the 44-byte three-object arena pins envelope identity `1520342505`, page identities `934788601`, `1001962162`, and `1407376619`, complete identity `551151124`, native exit `130`, and atomic forged-identity plus full-capacity failures.
