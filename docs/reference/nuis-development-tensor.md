...
* `standard-library/ns-nova/application-rendering-framework`: `active/99`, optional;
  compiled PixelMagic WGSL is projected from verified YIR into content-addressed MSL and registered requests. Provider-neutral IPC v2 carries typed runtime draw scalars; Shader owns their schema and the registered adapter owns admission. The example derives vertex counts `3/2/3` and instance counts `1/2/3` from profile values and frame state; profile validation follows count operand dependencies, not only direct edges. A separate-process M2 regression after an AOT build proves full/clear/full real GPU coverage for three/two/three vertices and identical first/third coverage. One Nsdb session, worker PID, and adapter cache span sequences `0/1/2`, with independent GLM release and same-command physical completion. Bound draws use borrowed validation without CPU reference rasterization and record actual RGBA8 events. Result-stream v2 binds canonical arguments into its source/target/pixel/completion identity; changed runtime arguments do not consume a replay frame and persisted argument drift breaks the stream hash. Live/replayed pixels match byte-for-byte. The current unbound Metal projection admits 1-4 vertices and 1-256 instances at fixed extent; resource bindings and other pass formats/topologies fail closed, while legacy packet values are not uploaded as GPU uniforms. Unsupported canonical vertex-body changes are rejected instead of replaced by fullscreen geometry. Typed uploads, whole-app continuity, multi-target scheduling, self-contained injection, and non-Metal parity remain open
