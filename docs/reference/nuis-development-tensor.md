...
* `standard-library/ns-nova/application-rendering-framework`: `active/99`, optional;
  compiled PixelMagic WGSL is projected from verified YIR into content-addressed MSL and registered requests. Provider-neutral IPC v2 carries runtime draw scalars and small owned immutable resources under the unchanged 256-byte argument limit. Optional BindingSet operands survive frontend/NIR ownership, optimization, call/profile traversal and YIR lowering. Shader validates four finite f32 values from a tuple or canonical indexed Tuple aggregate; the adapter admits the compiled group-zero fragment slot, and native Metal reflection checks a read-only float4 buffer before its 16-byte upload. The separate-process M2 regression after an AOT build proves red/clear/blue frames from Nuis tint data with vertices `3/2/3`, instances `1/2/3`, identical first/third coverage and different uniform-controlled channels. One session, worker PID and adapter executable cache retain `0/1/2`, `compiled/hit/hit`, same-command completion and independent GLM output release. Result-stream v2 binds resource name, type, shape, little-endian bytes and content identity alongside source, target, pixels and completion. Live/replay pixels match byte-for-byte; changed resources do not consume replay frames, and slot/code-capability drift cannot replace persisted evidence. Offline execution cannot invent uniform defaults. Legacy packets remain structural CPU preview input, not GPU uniforms. This closes one immutable f32x4 resource path, not general resource upload: large buffers/textures, fixed extent, whole-app continuity, multi-target scheduling, self-contained injection and non-Metal parity remain open
