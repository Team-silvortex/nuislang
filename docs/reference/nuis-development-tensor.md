...
With the driver, stage-neutral selection, and differential/reproducibility gate at `usable/99`, while data model v9 remains `usable/98`, deterministic coordinate ordering advances to `standard-library/std/compiler-data-model`; representation-sidecar aggregate binding, authorization consumption, a signed rollback chain, chunked individual typed payloads, and wider aggregate lowering remain open without changing subset v8.
