...
With the compiler data model, driver, stage-neutral selection, and differential/reproducibility gate all at `usable/99`, deterministic coordinate ordering remains at `compiler-toolchain/bootstrap/stage0-stage1-driver`; fresh-source capability v1 now binds the immutable signed-successor identity to one exact candidate-owned 56-byte source-to-token/AST/NIR/YIR execution with no stage0 handoff or runtime provider, so candidate-owned native materialization is now first, followed by representation-sidecar aggregate binding, nested full-arena forwarding, wider source coverage, and wider aggregate lowering without changing subset v8.
...
* preserve the foundation-through-`beta-0.9.*`, migration-from-`beta-0.10.*`,
  and `gamma-0.5.*` through `gamma-0.10.*` completion window without confusing roadmap agreement with implementation completion
