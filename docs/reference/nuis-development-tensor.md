...
The v11 candidate trust chain binds handoff v2 and keeps the stage0/stage1 driver at `usable/98`; a checked-in Linux amd64 generation now binds two remote cache-bypassed clean builds to the challenge-bound, pinned-registry Ed25519 channel and moves the differential/reproducibility gate to `usable/98`. Wrong challenge and registry-pin replay fail closed, the private key remains outside the repository, and replacement authority remains false.
...
With stage-neutral selection, the driver, reproducibility, and data model v9 all at `usable/98`, deterministic coordinate ordering advances to `compiler-toolchain/bootstrap/stage0-stage1-driver`; data-model growth remains open for chunked individual typed payloads and wider aggregate lowering without changing subset v8.
