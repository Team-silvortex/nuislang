...
The v11 candidate trust chain binds handoff v2 and keeps the stage0/stage1 driver at `usable/98`; two local cache-bypassed clean builds plus the challenge-bound, pinned-registry Ed25519 channel move the differential/reproducibility gate to `usable/97`, while one separately provisioned attester result remains open.
...
With stage-neutral selection and the driver at `usable/98` and reproducibility at `usable/97`, deterministic selection returns to compiler-data-model at `usable/96`; its next proof is paged typed payload storage beyond the bounded sixteen-byte column.
