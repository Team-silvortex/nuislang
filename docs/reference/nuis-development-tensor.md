...
With the driver at `usable/99` and stage-neutral selection, differential/reproducibility, and data model v9 still at `usable/98`, deterministic coordinate ordering advances to `developer-system/bootstrap/differential-reproducibility-gate`; authorization consumption, a signed rollback chain, chunked individual typed payloads, and wider aggregate lowering remain open without changing subset v8.
