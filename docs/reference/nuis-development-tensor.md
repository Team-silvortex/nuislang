...
With the compiler data model, driver, stage-neutral selection, and differential/reproducibility gate all at `usable/99`, deterministic coordinate ordering remains at `compiler-toolchain/bootstrap/stage0-stage1-driver`; giving a Nuis-owned stage1 candidate the same canonical compile capability before any signed generation-three forward transition is now the first open boundary, followed by representation-sidecar aggregate binding, nested full-arena forwarding, and wider aggregate lowering without changing subset v8.
