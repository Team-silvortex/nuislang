...
With the compiler data model, driver, stage-neutral selection, and differential/reproducibility gate all at `usable/99`, deterministic coordinate ordering remains at `compiler-toolchain/bootstrap/stage0-stage1-driver`; candidate successor v1 now signs the direct no-runtime-provider front-end proof into generation three without mutating generation two, capability v1, or preselection v1, so candidate-owned fresh-source processing is now first, followed by native materialization ownership, representation-sidecar aggregate binding, nested full-arena forwarding, and wider aggregate lowering without changing subset v8.
