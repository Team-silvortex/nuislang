...
With the driver and differential/reproducibility gate at `usable/99`, while stage-neutral selection and data model v9 remain `usable/98`, deterministic coordinate ordering advances to `language-core/nuisc/stage-neutral-ir-boundary`; representation-sidecar aggregate binding, authorization consumption, a signed rollback chain, chunked individual typed payloads, and wider aggregate lowering remain open without changing subset v8.
