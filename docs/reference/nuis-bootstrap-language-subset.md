...
`nuis-bootstrap-language-subset-v1`. Its machine-readable inventory is
[nuis-bootstrap-language-subset-v1.toml](nuis-bootstrap-language-subset-v1.toml),
...
1. Every project-local AST module obeys the v1 allowlist.
...
V1 permits only `mod cpu` compiler modules. Local CPU modules may import each
other; external imports are limited to `CorePrelude`, `StdLanguageCore`, and
`StdTextContracts`.
...
V1 rejects these capabilities even when the wider language supports them:
