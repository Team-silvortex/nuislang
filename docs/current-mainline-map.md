...
`frozen compiler subset -> compiler-owned data -> producer-neutral structural handoff -> first Nuis stage1 candidate -> stage0/stage1 driver -> fail-closed canonical plus selected-representation differential reports -> two-clean-build aggregate -> pinned external attestation -> separate replacement authorization`
