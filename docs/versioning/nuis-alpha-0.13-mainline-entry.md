...
This file is the current short entry point for the `alpha-0.13.*` line.
...
`alpha-0.13.*` is where nuis treats the development tensor as the default
...
* `alpha-0.13.*` makes tensor-guided closure hardening the current working mode
* current docs should say `alpha-0.13.*` for present-tense work and link older
