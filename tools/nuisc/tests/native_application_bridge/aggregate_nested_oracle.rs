...
#[test]
fn nested_tree_admission_retains_source_and_call_depth_budgets() {
    let fixture = Fixture {
        count: 1,
        op: "",
        keep_first_else: true,
    ...
    // Scoped iteration helpers consume the separate native call-depth budget;
    // a source-admitted tree need not fit it after adding callback/helper frames.
    let project = Project::with_source(&Tree::chain(32).source(fixture, true, true));
    let module = nuisc::pipeline::compile_project(&project.0).unwrap().yir;
    let error = emit_registered(&module, "counter").unwrap_err();
    assert!(error.contains("call-depth bound"), "{error}");
    for depth in [33, 40] {
        let project = Project::with_source(&Tree::chain(depth).source(fixture, true, true));
        assert!(
            nuisc::pipeline::compile_project(&project.0).is_err(),
            "source nesting above 32 must be rejected: {depth}"
        );
    }
}
...
fn nested_aggregate_updates_compose_with_typed_lifecycle() {
    assert_native_parity(include_str!("aggregate_nested_oracle_loops.ns"), true);
}
