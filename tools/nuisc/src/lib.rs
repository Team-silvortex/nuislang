...
pub fn std_net_sample_navigation_brief(domain: &str) -> Option<&'static str> {
    match domain {
        "network" => {
            Some("profile_core -> transport_edge -> socket_edge -> control_edge -> protocol_edge -> http_edge -> result_spine -> task_spine -> session")
        }
        _ => None,
    }
}
