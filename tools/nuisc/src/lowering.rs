...
fn collect_self_recursive_functions(module: &NirModule) -> BTreeSet<String> {
    module
        .functions
        .iter()
...
fn lower_direct_call_helper_function(
...
struct LoweringState<'a> {
    ...
    direct_call_functions: BTreeSet<String>,
    pure_helpers: BTreeSet<String>,
    value_counter: usize,
    print_counter: usize,
    await_counter: usize,
...
struct PreparedChainedWhile {
    binding_name: String,
    limit: NirExpr,
    step: NirExpr,
    compare: PreparedLoopCompare,
    step_kind: PreparedLoopStepKind,
    carries: Vec<PreparedCarryUpdate>,
}

struct PreparedFlowWhile {
    binding_name: String,
    limit: NirExpr,
    step: NirExpr,
    compare: PreparedLoopCompare,
...
struct PreparedCarryUpdate {
    binding_name: String,
    kind: PreparedCarryUpdateKind,
}

const TAIL_RECURSIVE_PREV_CURRENT_BINDING: &str = "__tailrec_prev_current";
const TAIL_RECURSIVE_PREV_CARRY_BINDING_PREFIX: &str = "__tailrec_prev_carry_";
...
struct PreparedLoopCarryCondition {
...
fn tail_recursive_prev_carry_binding(index: usize) -> String {
...
fn parse_loop_carry_branch_source(
    carry_name: &str,
    expr: &NirExpr,
    binding_name: &str,
    carries: &[PreparedCarryUpdate],
...
mod tests {
    ...
    fn lowers_flow_breaking_while_into_loop_while_i64_flow_chain() {
        ...
        let loop_node = yir
            ...
            .iter()
            .find(|node| {
                node.op.module == "cpu" && node.op.instruction == "loop_while_i64_flow_chain"
            })
