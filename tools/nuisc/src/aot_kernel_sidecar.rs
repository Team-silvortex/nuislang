...
pub(crate) fn render_domain_build_unit_kernel_ir_sidecar(
    ...
    match profile.profile_key {
        ...
        "cuda.nvidia-gpu" => {
            let asset = crate::kernel_code_asset::select_kernel_code_asset(profile.profile_key)
                .expect("registered CUDA lowering requires its PTX code asset");
            out.push_str("primary = \"grid\"\n");
            out.push_str("fallback = \"batch\"\n");
            out.push_str("[resource_bindings]\n");
            ...
            out.push_str(&format!("virtual_arch = \"{}\"\n", asset.target));
            out.push_str(&format!(
                "source_fnv1a64 = \"{}\"\n",
            ...
            out.push_str(&format!(
                ...
                escape_toml_string(
                    std::str::from_utf8(asset.bytes)
                        .expect("registered PTX code assets must remain UTF-8"),
                )
            ));
        }
        "cpu-fallback.cpu-host" => {
