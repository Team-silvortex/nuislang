...
pub fn validate_manifest_for_packaging(manifest: &NustarPackageManifest) -> Result<(), String> {
    ...
    crate::registry::linker_resolver_registrations(std::slice::from_ref(manifest))?;
    Ok(())
}
...
pub(super) fn render_manifest(manifest: &NustarPackageManifest) -> String {
    format!(
        ...
        render_array(&manifest.linker_resolver_providers),
        render_array(&manifest.linker_symbol_versions),
...
pub(super) fn parse_manifest_text(
    ...
    Ok(NustarPackageManifest {
        ...
        .unwrap_or_default(),
        linker_resolver_providers: parse_optional_string_array(source, "linker_resolver_providers")
            .unwrap_or_default(),
        linker_symbol_versions: parse_optional_string_array(source, "linker_symbol_versions")
            .unwrap_or_default(),
