        ...
        for contract in contracts {
            ...
            assert!(
                contract
                    ...
                    .iter()
                    .any(|item| item
                        == "host_symbol=network.bind_udp:host_network_bind_udp_datagram"),
                "missing network.bind_udp host symbol metadata in {} contract",
                contract.kind
            );
            assert!(
                contract.required_metadata.iter().any(
                    |item| item == "host_symbol=network.accept_owned:host_network_accept_owned"
                ),
