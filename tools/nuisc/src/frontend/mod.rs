...
use self::metadata::{
    helper_visible_struct_annotations, hidden_private_field_count, lower_ast_attributes,
    ModuleConstValue,
};
use self::return_inference::infer_missing_function_return_type;
use aliases::*;
use nuis_semantics::model::{
    nir_expr_effect_class, AstBinaryOp, AstExpr, AstFunction, AstImplDef, AstMatchArm,
    AstMatchPattern, AstModule, AstParam, AstStmt, AstTypeAlias, AstTypeRef, AstVisibility,
    NirBinaryOp, NirExpr, NirExprEffectClass, NirExternFunction, NirExternInterface, NirFunction,
    NirGenericParam, NirImplDef, NirImplMethod, NirKernelAxis, NirKernelMapOp, NirKernelZipOp,
    NirModule, NirResultFamily, NirResultStage, NirStmt, NirStructDef, NirStructField, NirTraitDef,
    NirTraitMethodSig, NirTypeAlias, NirTypeRef, NirUse, NirVisibility, NirWindowMode,
};
use types::*;
...
fn lower_call_expr_with_async(
    ...
    match callee {
        "i32_from_i64" => {
            ...
            Ok(NirExpr::CastI64ToI32(Box::new(lowered)))
        }
        "spawn" => {
            if current_domain != "cpu" {
                return Err(
        ...
        "network_profile_bind_core" => {
            ...
            Ok(NirExpr::NetworkProfileBindCoreRef { unit: unit.clone() })
        }
        ...
        "network_profile_timeout_budget" => {
            ...
            Ok(NirExpr::NetworkProfileTimeoutBudgetRef { unit: unit.clone() })
        }
        "network_profile_retry_budget" => {
            ...
            Ok(NirExpr::NetworkProfileRetryBudgetRef { unit: unit.clone() })
        }
