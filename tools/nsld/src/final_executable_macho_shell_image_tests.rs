...
fn serializes_a_deterministic_private_arm64_shell_image() {
    ...
    assert_eq!(first.bytes.len(), shell.code_signature_file_offset);
    assert_eq!(first.report.shell_image_span_bytes, first.bytes.len());
    assert_eq!(
        first.report.status,
        "private-image-serialized-with-code-signature-boundary"
    );
    assert_eq!(first.report.code_signature_status, "payload-pending");
    ...
    assert_eq!(read_u32(&first.bytes, signature.command_offset + 12), 0);
