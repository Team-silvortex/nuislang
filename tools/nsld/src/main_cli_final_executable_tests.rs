...
#[test]
fn parses_private_image_admission_verify_command() {
    let command = parse_args(
        vec![
            "verify-final-executable-private-image-admission".to_owned(),
            "out".to_owned(),
            "--json".to_owned(),
        ]
        .into_iter(),
    ...
    assert_eq!(
        command,
        Ok(Command::VerifyFinalExecutablePrivateImageAdmission {
            input: PathBuf::from("out"),
            json: true,
        })
    );
...
#[test]
fn parses_private_image_publication_apply_and_json_flags() {
    let command = parse_args(
        vec![
            "final-executable-private-image-publication".to_owned(),
            "out".to_owned(),
            "--apply".to_owned(),
            "--json".to_owned(),
        ]
    ...
    assert_eq!(
        command,
        Ok(Command::FinalExecutablePrivateImagePublication {
            input: PathBuf::from("out"),
            json: true,
            apply: true,
        })
