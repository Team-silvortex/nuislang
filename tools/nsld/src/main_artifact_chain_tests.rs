...
fn artifact_chain_report_lists_registered_stages_and_optional_tail() {
    ...
    assert_eq!(
        report.next_action_command_reason.as_deref().map(|reason| {
            reason.contains("first artifact-chain advisory")
                && reason.contains("final-executable-writer-input")
        }),
        Some(true)
    );
    assert!(report
...
fn artifact_chain_next_optional_stage_advances_through_final_executable_tail() {
    ...
    assert_eq!(
        after_prepare.next_action_command_id.as_deref(),
        Some("emit-final-executable-pipeline")
    );
    assert_eq!(
        after_prepare.next_action_source.as_deref(),
        Some("optional")
    );
    assert!(after_prepare.next_action_available);
    assert_eq!(
