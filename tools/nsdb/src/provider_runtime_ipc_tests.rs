...
fn invalid_events_and_disconnect_never_reach_device_execution() {
    ...
    for event in [
        ...
        let result = dispatch_loop(&mut Cursor::new(wire), &target, || {
            panic!("invalid event executed device")
        });
        assert!(result.is_err());
    }
    assert!(
        dispatch_loop(&mut Cursor::new(Vec::<u8>::new()), &target, || panic!(
...
fn zero_draw_lifecycle_closes_without_invoking_provider() {
    ...
    let (count, output) = dispatch_loop(&mut Cursor::new(wire), &target(), || {
