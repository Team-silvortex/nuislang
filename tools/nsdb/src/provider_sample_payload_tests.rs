...
fn sample_record(input_evidence: &str) -> NsdbDeviceProviderSampleRecordInfo {
    NsdbDeviceProviderSampleRecordInfo {
        ...
        provider_runner_adapter_id: "metal.apple-silicon-gpu.real-device".to_owned(),
...
fn real_device_payload_carries_pixelmagic_output_bytes() {
    ...
    assert!(payload.contains("provider_code_asset_identity_status = \"not-applicable\""));
    assert!(payload.contains("provider_code_asset_identity_set_status = \"not-applicable\""));
    assert!(payload.contains("provider_code_asset_identity_set_count = \"0\""));
}
...
fn real_device_payload_carries_released_transport_receipt() {
    ...
    let payload =
        render_real_device_provider_output_payload(&record, &adapter, &[], &[receipt], "", None);
