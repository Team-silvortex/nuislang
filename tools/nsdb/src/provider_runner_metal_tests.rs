...
use super::{
    ...
    execute_gray8_threshold, execute_u32_copy_input,
};
#[cfg(target_os = "macos")]
...
fn registered_shader_asset(asset_id: &str) -> (Vec<u8>, String) {
    ...
    let asset = nuisc::registry::code_asset_registrations(&root, &manifest)
        .unwrap()
        .into_iter()
...
fn executes_generated_u32_copy_msl_from_opaque_carrier_bytes() {
    let (metal_source, entry) = registered_shader_asset("shader.metal.copy-u32.msl");
    let source_path = std::env::temp_dir().join(format!(
        "nuis-metal-u32-copy-source-{}-{}.metal",
        std::process::id(),
        std::thread::current().name().unwrap_or("test")
    ));
    std::fs::write(&source_path, metal_source).unwrap();
    let input = ProviderCarrierInput::OpaqueBytes {
        handle: "memory:metal-u32-copy-test".to_owned(),
        bytes: [1u32, 8, 13, 21]
            .into_iter()
    ...
    let execution =
        execute_u32_copy_input(&input, &source_path, &entry).expect("generated Metal u32 copy");
