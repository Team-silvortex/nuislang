...
fn self_contained_nsb_route_moves_from_nsld_drive_to_run_artifact_handoff() {
    ...
    assert!(
        native_entry_stdout.contains("\"ready\":true")
            ...
            && native_entry_stdout.contains("\"dispatch_resolution_protocol\":\"nuis-runtime-dispatch-import-resolution-v1\"")
            && native_entry_stdout.contains("\"dispatch_resolution_status\":\"resolved\"")
            && native_entry_stdout.contains("\"dispatch_import_declared\":true")
            && native_entry_stdout.contains("\"dispatch_import_identity_hash\":\"0x")
            && native_entry_stdout.contains("\"dispatch_table_identity\":")
            && native_entry_stdout.contains("\"dispatch_capability_mask\":3")
            && native_entry_stdout.contains("\"dispatch_slot\":2")
            && native_entry_stdout.contains("\"dispatch_status_code\":0")
            && native_entry_stdout.contains("\"dispatch_acknowledged\":true")
