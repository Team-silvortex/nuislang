...
const REGISTRATION_ID: &str = "official.shader.metal-add-xor-pair-u32";
const METADATA_SELECTOR: &str = "official.shader:provider-sample=metal-add-xor-pair-u32";
const ASSET_ID: &str = "shader.metal.add-xor-pair-u32.msl";
const ENTRY: &str = "nuis_metal_add_xor_pair_u32";
const LOWERING_TARGET: &str = "metal.apple-silicon-gpu";
const INPUT_FILE: &str = "nuis.shader.metal.add-xor-pair-u32.left.u32.bin";
const RIGHT_FILE: &str = "nuis.shader.metal.add-xor-pair-u32.right.u32.bin";
const SUM_FILE: &str = "nuis.shader.metal.add-xor-pair-u32.sum.expected.u32.bin";
const XOR_FILE: &str = "nuis.shader.metal.add-xor-pair-u32.xor.expected.u32.bin";
...
pub(crate) fn registration() -> DeviceSampleInputRegistration {
    DeviceSampleInputRegistration {
        ...
        supports: |backend, device| backend == "metal" && device == "apple-silicon-gpu",
...
fn sample_evidence(_base: &str) -> String {
    let asset = asset().expect("Shader Nustar Metal fan-out asset must be registered");
    ...
    render_u32_prefixed_request_evidence(U32RequestEvidence {
        prefix: "provider_",
        provider_family: "metal:apple-silicon-gpu",
        kernel_id: "shader.metal.add-xor-pair-u32",
...
fn persist_payloads(output_dir: &Path, evidence: &[&str]) -> Result<(), String> {
    ...
    let actual = fs::read(output_dir.join(&asset.file_name))
        .map_err(|error| format!("failed to read Nuis-emitted Metal fan-out asset: {error}"))?;
    ...
    for (name, bytes) in [
        ...
        fs::write(output_dir.join(name), bytes)
            .map_err(|error| format!("failed to persist Metal fan-out payload: {error}"))?;
    }
    Ok(())
}
...
fn resolve_code_asset_evidence(output_dir: &Path, evidence: &str) -> Result<String, String> {
    ...
    let actual = fs::read(output_dir.join(&asset.file_name))
        .map_err(|error| format!("failed to read Nuis-emitted Metal fan-out asset: {error}"))?;
    validate_asset(&asset, &actual)?;
    validate_code_asset_request_evidence("Metal fan-out", &asset, &actual, evidence, "provider_")?;
    let selection =
...
fn validate_asset(
    ...
    if bytes != asset.bytes {
        return Err(
            "Nuis-emitted Metal fan-out asset does not match registry ownership".to_owned(),
        );
    }
    let source = std::str::from_utf8(bytes)
        .map_err(|_| "Nuis-emitted Metal fan-out asset is not UTF-8".to_owned())?;
    for proof in [
        "nuis-yir.shader.backend-lowering-plan.v1",
        "msl:metal-gpu",
        ENTRY,
    ] {
        if !source.contains(proof) {
            return Err(format!(
                "Nuis-emitted Metal fan-out asset is missing proof `{proof}`"
            ));
        }
    }
    Ok(())
...
fn asset() -> Result<nuisc::registry::NustarCodeAssetRegistration, String> {
    ...
    nuisc::registry::code_asset_registration_by_id(root, &manifest, ASSET_ID)?
        ...
        .ok_or_else(|| "Shader Nustar Metal fan-out code asset is not registered".to_owned())
}
