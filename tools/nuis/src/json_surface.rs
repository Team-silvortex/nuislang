...
pub(crate) fn append_workflow_contract_json_fields(
    out: &mut String,
    ...
    crate::surface_render::append_json_field_strings(
        out,
        vec![
    ...
    if include_project_compile {
        crate::surface_render::append_json_field_strings(
            out,
            vec![
                crate::json_field("project_compile_workflow", frontdoor.workflow_brief),
                crate::json_field("project_compile_samples", frontdoor.workflow_samples),
            ],
        );
    }
    ...
    if include_debug {
        crate::surface_render::append_json_field_strings(
            out,
            vec![
                crate::json_field("debug_workflow", crate::debug_workflow_brief()),
                crate::json_field("debug_samples", crate::debug_workflow_samples_brief()),
            ],
        );
    }
}
...
pub(crate) fn append_public_surface_summary_json_fields(
    out: &mut String,
    records: &[PublicSurfaceModuleRecord],
) {
    let public_extern_count = records
...
pub(crate) fn append_project_plan_json_fields(
    out: &mut String,
    plan: &nuisc::project::ProjectCompilationPlan,
) {
    crate::surface_render::append_json_field_strings(
...
pub(crate) fn append_project_check_summary_json_fields(
    out: &mut String,
    abi_checks: &[nuisc::project::ProjectAbiSelectionCheck],
    registry_checks: &[nuisc::registry::ProjectDomainRegistryCheck],
    lowering_checks: &[nuisc::project::ProjectLoweringSelectionView],
...
pub(crate) fn append_galaxy_lock_json_fields(
    out: &mut String,
    status: Result<galaxy::VerifiedGalaxyLock, String>,
    lock_path: &Path,
    declared_dependencies: &[String],
