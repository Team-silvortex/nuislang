...
use crate::artifact_nsdb_replay_cursor_lineage_repair::{
    validate_repair_journal, FILE_NAME as REPAIR_JOURNAL_FILE_NAME,
};
...
pub(crate) fn read_debugger_cursor_lineage(output_dir: &Path) -> DebuggerCursorLineageMirror {
    ...
    let (entry_count, latest_hash) = match validate_lineage(
        ...
        expected_proof_hash,
        &dispatch_identity.identity_hash,
    ) {
        Ok(summary) => summary,
        Err(first_blocker) => {
    ...
    let repair = read_repair_journal(
        output_dir,
        &path,
        &latest_hash,
        &dispatch_identity.identity_hash,
    ...
    DebuggerCursorLineageMirror {
        ...
        provider_dispatch_identity_hash: (dispatch_identity.identity_hash != "none")
            .then_some(dispatch_identity.identity_hash),
        first_blocker: None,
        next_action: None,
        next_command: None,
        repair,
...
fn read_repair_journal(
    ...
    let Some(summary) = validate_repair_journal(
        &source,
        lineage_path,
        lineage_hash,
...
fn validate_lineage(
    ...
    if field(source, "provider_dispatch_identity_hash").as_deref()
        != Some(expected_provider_dispatch_identity_hash)
    {
        return Err("lineage-provider-dispatch-identity-mismatch");
    }
    ...
    if field(cursor_source, "provider_dispatch_identity_hash").as_deref()
        != Some(expected_provider_dispatch_identity_hash)
    {
        return Err("lineage-cursor-provider-dispatch-identity-mismatch");
    }
...
mod tests {
    ...
    fn rejects_lineage_from_another_provider_dispatch_identity() {
        ...
        fs::write(
            root.join(LINEAGE_FILE_NAME),
            format!(
                "protocol = \"{LINEAGE_PROTOCOL}\"\nidentity_contract = \"nsdb-yir-replay-identity-v1\"\nfinal_image_binding_proof_hash = \"{TEST_PROOF_HASH}\"\nprovider_dispatch_identity_hash = \"0x0123456789abcdef\"\ncursor_path = \"{}\"\nentry_limit = 8\nentry_count = 1\n\n[[entry]]\nsequence = 0\nprevious_hash = \"none\"\ncurrent_hash = \"{}\"\nafter_frame_id = \"frame-0\"\nnext_frame_id = \"frame-1\"\nfinal_image_binding_proof_hash = \"{TEST_PROOF_HASH}\"\nprovider_dispatch_identity_hash = \"0x0123456789abcdef\"\n",
                cursor_path.display(),
                fnv1a64_hex(cursor.as_bytes())
            ),
        )
        .unwrap();
