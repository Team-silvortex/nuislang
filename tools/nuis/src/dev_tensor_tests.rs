...
fn dev_tensor_summary_reports_three_axes_and_cells() {
    ...
    assert_eq!(
        ...
        "cargo check -q -p nsdb && cargo test -q -p nuisc filtered_code_asset_lookup_does_not_materialize_unselected_entries && cargo test -q -p nuisc shader_spirv_emitter && cargo test -q -p nuisc shader_msl_emitter && cargo test -q -p nuisc materializes_registered_vulkan_spirv_and_contribution_table && cargo test -q -p nuisc materializes_registered_msl_from_canonical_wgsl_source && cargo test -q -p nsdb vulkan_session_plan_accepts_registered_u32_shapes && cargo test -q -p nsdb vulkan_session_plan_accepts_verified_dependency_input && cargo test -q -p nuis artifact_device_sample_shader_vulkan && cargo test -q -p nsdb executes_generated_u32_copy_msl_from_opaque_carrier_bytes && cargo test -q -p nsdb executes_generated_u32_add_msl_from_opaque_carrier_bytes && cargo test -q -p nsdb executes_generated_u32_sub_msl_from_opaque_carrier_bytes && cargo test -q -p nsdb executes_generated_u32_mul_msl_from_opaque_carrier_bytes && cargo test -q -p nsdb executes_generated_u32_xor_msl_from_opaque_carrier_bytes && cargo test -q -p nuis artifact_device_sample_shader_metal && cargo test -q -p nuis --test official_galaxy_hetero_smoke shader_metal_generated_msl_sample_executes_provider_output -- --nocapture && cargo test -q -p nuis --test official_galaxy_hetero_smoke shader_metal_generated_add_msl_sample_executes_provider_output -- --nocapture && cargo test -q -p nuis --test official_galaxy_hetero_smoke shader_metal_generated_sub_msl_sample_executes_provider_output -- --nocapture && cargo test -q -p nuis --test official_galaxy_hetero_smoke shader_metal_generated_mul_msl_sample_executes_provider_output -- --nocapture && cargo test -q -p nuis --test official_galaxy_hetero_smoke shader_metal_generated_chain_msl_sample_executes_provider_graph -- --nocapture && cargo test -q -p nuis link_plan_provider_dispatch_identity && cargo test -q -p nsld final_image_loader_verifies_vulkan_provider_dispatch_without_backend_branch && cargo test -q -p nsdb vulkan_completion_authority && cargo test -q -p nsld validates_verified_or_not_applicable_selection && cargo test -q -p nuis dev_tensor; remote linux: cargo test -q -p nuis --test official_galaxy_hetero_smoke linux_vulkan -- --nocapture"
    );
    assert!(summary
