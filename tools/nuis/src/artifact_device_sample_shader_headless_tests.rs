...
fn headless_metal_session_uses_shared_lifecycle_without_appkit() {
    ...
    let prepared = prepare_runtime_provider_results(&output.0)
        .unwrap()
        .unwrap();
    let headless = output.0.join("headless");
    let build = Command::new("cargo")
        .current_dir(&root)
        .args(["run", "-p", "yir-pack-aot", "--"])
        .arg(&prepared.source_yir_path)
        .arg(&headless)
        .arg("--headless")
    ...
    let binary = headless.join(prepared.source_yir_path.file_stem().unwrap());
