...
pub(crate) fn render_device_provider_sample_manifest_toml<'a>(
    ...
    let code_asset_table =
        crate::artifact_code_asset_contribution_table::verify_compiled_code_asset_contribution_table(
            output_dir,
        )?;
    let code_asset_table_evidence =
        crate::artifact_code_asset_contribution_table::render_verified_table_evidence(
            code_asset_table.as_ref(),
        );
    let records = samples
...
pub(crate) fn persist_device_sample_input_payloads<'a>(
    ...
    crate::artifact_device_sample_witsage::persist_assets_if_requested(output_dir, &evidence)?;
    Ok(())
}
...
fn input_evidence(
    ...
    if backend_family == Some("coreml") && target_device == Some("apple-ane") {
        return crate::artifact_device_sample_witsage::input_evidence(&base);
    }
    base
}
