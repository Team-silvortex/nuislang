...
mod dev_tensor_render_protocol;
mod dev_tensor_status;
mod dev_tensor_task_card_lineage;
mod dev_tensor_task_card_lineage_render;
mod digest_sha256;
mod galaxy;
mod galaxy_command;
mod help;
...
fn run() -> Result<std::process::ExitCode, String> {
    match cli::parse_args(std::env::args().skip(1))? {
        ...
        cli::CommandKind::BootstrapCandidateCompileCapability(input) => {
            bootstrap_candidate_compile_capability::handle_bootstrap_candidate_compile_capability(
                input,
            )?;
        }
        ...
        cli::CommandKind::RunArtifact { input, json, frame_output, window_session, application_session, native_session } => {
            if let Some(arguments) = native_session {
                artifact_runtime_command::handle_run_artifact_with_native_session(input, arguments)?;
            } else if let Some(script) = application_session {
