...
fn vulkan_chain_u32_evidence(_base: &str) -> String {
    ...
    let mul = vulkan_asset(MUL_SAMPLE).expect("Shader Nustar Vulkan mul asset must be registered");
    let identity = vulkan_chain_code_asset_identity(&add, &mul)
...
fn render_dependency_count_zero(prefix: &str) -> String {
    format!("{prefix}dependency_contract=nuis-provider-request-dependency-v1;{prefix}dependency_count=0")
}

fn render_chain_dependency(prefix: &str) -> String {
    format!(
        "{prefix}dependency_contract=nuis-provider-request-dependency-v1;{prefix}dependency_count=1;{prefix}dependency_0_producer_request_id=shader.vulkan.chain.add-u32;{prefix}dependency_0_producer_output_buffer=output.values;{prefix}dependency_0_consumer_input_buffer=input.values;{prefix}dependency_0_transport_contract=nuis-provider-edge-transport-v1;{prefix}dependency_0_transport_ownership_token=glm:provider-edge:shader.vulkan.chain.add-u32:output.values->shader.vulkan.chain.mul-u32:input.values;{prefix}dependency_0_transport_staging_mode=auto;{prefix}dependency_0_transport_producer_clock_evidence=provider-clock:request-0:completed;{prefix}dependency_0_transport_consumer_clock_evidence=provider-clock:request-1:dispatch-ready"
    )
}
...
fn persist_vulkan_payloads(output_dir: &Path, evidence: &[&str]) -> Result<(), String> {
    if evidence.iter().any(|item| evidence_is_chain(item)) {
        ...
        let mul = vulkan_asset(MUL_SAMPLE)?;
        for asset in [&add, &mul] {
...
fn resolve_vulkan_code_asset_evidence(output_dir: &Path, evidence: &str) -> Result<String, String> {
    ...
    let actual = fs::read(output_dir.join(&asset.file_name))
        .map_err(|error| format!("failed to read Nuis-emitted Vulkan SPIR-V asset: {error}"))?;
    validate_vulkan_code_asset(&asset, &actual)?;
    validate_vulkan_request_asset_evidence(&asset, &actual, evidence, "provider_")?;
    let selection =
    ...
    let selection_evidence = selection
        ...
        .map(|selection| -> Result<String, String> {
            validate_vulkan_contribution_selection(selection, evidence, "provider_")?;
...
fn resolve_vulkan_chain_code_asset_evidence(
    ...
    for (index, sample) in [(0usize, ADD_SAMPLE), (1, MUL_SAMPLE)] {
        ...
        let actual = fs::read(output_dir.join(&asset.file_name))
            .map_err(|error| format!("failed to read Nuis-emitted Vulkan SPIR-V asset: {error}"))?;
        validate_vulkan_code_asset(&asset, &actual)?;
        validate_vulkan_request_asset_evidence(&asset, &actual, evidence, &prefix)?;
        let selection =
            crate::artifact_code_asset_contribution_table::select_compiled_code_asset_contribution(
                output_dir,
                PACKAGE_ID,
        ...
        validate_vulkan_contribution_selection(&selection, evidence, &prefix)?;
