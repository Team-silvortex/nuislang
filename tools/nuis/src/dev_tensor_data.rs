...
pub(crate) const DEV_TENSOR_CELLS: &[DevTensorCell] = &[
    ...
    DevTensorCell {
        ...
        evidence: "final-executable-output reports normalized boundary status, materialization status, recommended next action, path presence, ownership, runnable candidate, blockers, Nuis/Nsld mirrors, self-contained relocation-safe images, container-loader handoff readiness, and first-payload execution metadata from the final output itself",
        next_step: "feed first-payload execution metadata into Nuis/run-artifact and the future nsdb YIR-level debugger handoff instead of keeping it Nsld-local",
    },
    DevTensorCell {
        ...
        evidence: "domain units, lowering targets, backend families, contract completeness status, contract drift checks, heterogeneous domain readiness, dispatch readiness, bridge materialization, and link-plan readiness consumption are visible through registered Nustar domain contracts",
        next_step: "feed registry dispatch readiness into nsld final-output blocker ordering instead of only Nuis workflow/link-plan readiness",
    },
    DevTensorCell {
        ...
        evidence: "std IO/filesystem/text/stdin examples, std_filesystem_smoke light lane, observable CLI smoke, direct cli_wc_demo argv/file/text-stats run, tooling docs, and std lane docs anchor the current host-backed CLI foundation",
        next_step: "turn the host-backed CLI workload into a reusable std report helper that can write an inspectable report file through the same argv/file/text path",
    },
    DevTensorCell {
        architecture: "standard-library",
        module: "pixelmagic",
    ...
    DevTensorCell {
        ...
        evidence: "control flow, generics, traits, pointers, floats, enums, error handling, inference, task_result_enum, generic trait-bound guarded-match, and GLM buffer language bootstrap smokes have staged CLI/IR/runtime coverage",
        next_step: "expand bootstrap-critical language smokes into trait-bound std-style helper workloads that combine Result, Buffer, lambdas, and pointer-heavy control flow in one project",
    },
    DevTensorCell {
