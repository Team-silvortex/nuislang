...
pub(crate) const DEV_TENSOR_CELLS: &[DevTensorCell] = &[
    ...
    DevTensorCell {
        ...
        next_step: "feed backend artifact candidate readiness into nsld final-output selection and multi-backend blocker ordering",
        blocker: "backend artifact candidates are visible in frontdoors but not yet selected by nsld as ordered final-output candidates",
        next_action: "feed backend artifact candidate readiness into nsld final-output selection and multi-backend blocker ordering",
        validation_command: "cargo test -q -p nuis dev_tensor && cargo check -q -p nuisc",
        expected_artifact: "nsld final-output reports ordered backend artifact candidates without knowing shader/kernel/network-specific logic",
    },
    DevTensorCell {
