...
mod tests {
    ...
    fn independently_detects_anchor_deletion_with_marker() {
        ...
        fs::create_dir_all(marker.parent().unwrap()).unwrap();
        assert!(matches!(
            enforce_at_paths(&anchor, &marker, "registry-v1", 4, &"d".repeat(64)),
            AnchorCheck::Accepted
        ));
    ...
    fn independently_migrates_existing_anchor_to_marker() {
        ...
        persist_anchor(&anchor, "registry-v1", 5, &"e".repeat(64)).unwrap();
        assert!(matches!(
            enforce_at_paths(&anchor, &marker, "registry-v1", 5, &"f".repeat(64)),
            AnchorCheck::Fork
        ));
        assert!(!marker.exists());
        assert!(matches!(
            enforce_at_paths(&anchor, &marker, "registry-v1", 5, &"e".repeat(64)),
            AnchorCheck::Accepted
        ));
        assert_eq!(
            read_marker(&marker)
