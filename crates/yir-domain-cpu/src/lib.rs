...
impl RegisteredMod for CpuMod {
    ...
    fn execute(
        ...
        if let Some(mut execution) = self.begin_execution(node, resource, state)? {
            for _ in 0..100_000 {
                match execution.resume(state, None)? {
                    yir_core::RegisteredExecutionStep::Continue => {}
                    yir_core::RegisteredExecutionStep::Complete(value) => return Ok(value),
                    yir_core::RegisteredExecutionStep::Call { .. } => {
                        return Err(format!(
                            "node `{}` requires a registered function execution context",
                            node.name
                        ));
                    }
                }
            ...
            return Err(format!(
                "node `{}` exceeded 100000 reference steps",
                node.name
            ));
        }
