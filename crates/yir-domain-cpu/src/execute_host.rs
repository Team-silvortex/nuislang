...
pub(crate) fn execute_cpu_host_node(
    ...
    let value: Value = match node.op.instruction.as_str() {
        ...
        "extern_call_owned_utf8" => Err(format!(
            "node `{}` owned FFI UTF-8 call requires native LLVM execution; the deterministic YIR interpreter does not expose host pointers",
            node.name
        )),
        "extern_call_owned_object" => Err(format!(
            "node `{}` owned FFI object call requires native LLVM execution; the deterministic YIR interpreter does not expose host pointers",
            node.name
        )),
        "input_i64" => {
