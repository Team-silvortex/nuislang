...
pub(crate) fn draw_control_panel_widgets(
    ...
    for (label, value, y, disabled, min_value, max_value, step_value) in slider_specs {
        put_text(rows, panel_left + 3, y, label);
        draw_slider(
            rows,
            slider_left,
            y,
        ...
        put_text(rows, slider_right + 2, y, &format!("{:>3}", value.min(127)));
        let meta = format!("{min_value}..{max_value} /{step_value}");
        put_text(rows, slider_left, y.saturating_sub(1), &meta);
        if disabled {
            put_text(rows, slider_right + 7, y, "off");
        }
    }
    ...
    put_text(rows, panel_left + 3, progress_y.saturating_sub(1), "frame");
    put_text(rows, panel_left + 3, progress_y, "PROGRESS");
    draw_slider(
        rows,
        slider_left,
        progress_y,
    ...
    put_text(rows, panel_left + 3, meter_y.saturating_sub(1), "energy");
    put_text(rows, panel_left + 3, meter_y, "METER");
    draw_slider(
        rows,
        slider_left,
        meter_y,
    ...
    put_text(rows, panel_left + 3, panel_bottom.saturating_sub(1), "gain");
    ...
    put_text(rows, text_left + 2, text_top + 1, &text_value);
    if packet.text_placeholder.rem_euclid(2) != 0 {
        put_text(rows, text_left + 2, text_top, "query");
    }
    if packet.text_read_only != 0 {
        put_text(rows, text_right.saturating_sub(6), text_top, "ro");
    }
    if packet.text_dirty != 0 {
        put_text(rows, text_right.saturating_sub(12), text_top, "dirty");
    }
    let caret_x =
    ...
    put_text(rows, select_left, select_y, labels[0]);
    put_text(rows, select_left + 7, select_y, labels[1]);
    if option_count >= 3 {
        put_text(rows, select_left + 13, select_y, labels[2]);
    }
    if option_count >= 4 {
        put_text(rows, select_left + 19, select_y, labels[3]);
    }
    if packet.select_multiple != 0 {
        put_text(rows, select_left, select_y.saturating_sub(1), "multi");
    }
    put_text(
        rows,
        select_left + 22,
    ...
    put_text(rows, selected_x, select_y, ">");

    let checkbox_y = panel_top + 6;
    let checkbox_left = button_left;
    put_text(rows, checkbox_left, checkbox_y, "CHECK");
    put_text(
        rows,
        checkbox_left,
        checkbox_y + 1,
    ...
    put_text(rows, radio_left, radio_y, "RADIO");
    ...
    if packet.radio_disabled != 0 {
        put_text(rows, radio_left + 8, radio_y, "off");
    }
    ...
    put_text(rows, textarea_left + 2, textarea_top, "notes");
    ...
    for line in 0..visible_lines {
        ...
        put_text(rows, textarea_left + 2, textarea_top + 1 + line, &text);
    }
    if packet.textarea_read_only != 0 {
        put_text(rows, textarea_right.saturating_sub(6), textarea_top, "ro");
    }
    ...
    put_text(rows, list_left, list_top, "list");
    ...
    for idx in 0..list_items {
        ...
        put_text(rows, list_left, list_top + 1 + idx, &row);
    }

    let table_left = panel_left + 50;
    let table_top = panel_bottom.saturating_sub(8);
    put_text(rows, table_left, table_top, "table");
    ...
    put_text(rows, table_left, table_top + 1, &header);
    for row_idx in 0..rows_count {
        ...
        put_text(rows, table_left, table_top + 2 + row_idx, &body);
    }

    let tree_left = panel_right.saturating_sub(26);
    let tree_top = panel_top + 15;
    put_text(rows, tree_left, tree_top, "tree");
    ...
    for idx in 0..node_count {
        ...
        put_text(rows, tree_left, tree_top + 1 + idx, &line);
    }

    let inspector_left = panel_right.saturating_sub(26);
    let inspector_top = panel_top + 4;
    put_text(rows, inspector_left, inspector_top, "inspector");
    put_text(
        rows,
        inspector_left,
        inspector_top + 1,
    ...
    for idx in 0..inspector_fields {
        ...
        put_text(rows, inspector_left, inspector_top + 2 + idx, &line);
    }

    let outline_left = panel_left + 3;
    let outline_top = panel_top + 17;
    put_text(rows, outline_left, outline_top, "outline");
    ...
    for idx in 0..outline_items {
        ...
        put_text(rows, outline_left, outline_top + 1 + idx, &line);
    }
