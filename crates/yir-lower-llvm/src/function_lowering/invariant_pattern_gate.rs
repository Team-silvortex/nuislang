...
pub(super) fn lower_false_invariant_post_flow_loop(
    ...
    let carry_stride = match (node.op.module.as_str(), node.op.instruction.as_str()) {
        ("cpu", "loop_while_i64_post_flow_chain" | "loop_while_scalar_post_flow_chain") => 2,
        (
            "cpu",
    ...
    if node.op.args.get(3).map(String::as_str) != Some("invariant_true") {
        return Ok(false);
    }
    ...
    let carry_start = if carry_stride == 2 {
        8
    } else {
    ...
    for (index, chunk) in node.op.args[carry_start..].chunks(carry_stride).enumerate() {
        let Some(carry_name) = chunk.first() else {
            return Ok(false);
        };
