...
pub fn carry_source_payload_len(kind: &str) -> Option<usize> {
...
#[cfg(test)]
mod tests {
    use super::carry_source_payload_len;

    #[test]
    fn accepts_single_state_scaled_source() {
        assert_eq!(carry_source_payload_len("add_scaled_prev_carry1"), Some(1));
        assert_eq!(
            carry_source_payload_len("add_scaled_prev_carry1_plus_invariant"),
            Some(2)
        );
    }

    #[test]
    fn retains_grouped_affine_source_arities() {
        assert_eq!(
            carry_source_payload_len("add_scaled_current_plus_current_plus_invariant"),
            Some(2)
        );
        assert_eq!(
    ...
    #[test]
    fn rejects_missing_or_invalid_state_terms() {
        assert_eq!(carry_source_payload_len("add_scaled_"), None);
        assert_eq!(carry_source_payload_len("add_scaled_unknown"), None);
    }
}
