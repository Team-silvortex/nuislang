...
impl RegisteredMod for NpuMod {
    fn module_name(&self) -> &'static str {
        "npu"
    }

    fn describe(&self, node: &Node, resource: &Resource) -> Result<InstructionSemantics, String> {
        require_npu_resource(node, resource)?;

        match node.op.instruction.as_str() {
            "target_config" => {
                if node.op.args.len() != 3 {
                    return Err(format!(
                        "node `{}` expects `npu.target_config <name> <resource> <arch> <runtime> <lane_width>`",
                        node.name
                    ));
                }
            ...
            "tensor" => {
                if node.op.args.len() != 3 {
                    return Err(format!(
                        "node `{}` expects `npu.tensor <name> <resource> <rows> <cols> <csv-elements>`",
                        node.name
                    ));
                }
            ...
            "fill" => {
                if node.op.args.len() != 3 {
                    return Err(format!(
                        "node `{}` expects `npu.fill <name> <resource> <rows> <cols> <value>`",
                        node.name
                    ));
                }
                parse_shape(node)?;
            ...
            "matmul" | "add_bias" => {
                if node.op.args.len() != 2 {
                    return Err(format!(
                        "node `{}` expects `npu.{} <name> <resource> <lhs> <rhs>`",
            ...
            "relu" => {
                if node.op.args.len() != 1 {
                    return Err(format!(
                        "node `{}` expects `npu.relu <name> <resource> <input>`",
                        node.name
                    ));
                }
                Ok(InstructionSemantics::pure(node.op.args.clone()))
            ...
            "print" => {
                if node.op.args.len() != 1 {
                    return Err(format!(
                        "node `{}` expects `npu.print <name> <resource> <input>`",
                        node.name
                    ));
                }
                Ok(InstructionSemantics::effect(node.op.args.clone()))
            }
            other => Err(format!("unknown npu instruction `{other}`")),
        }
    }
...
fn require_npu_resource(node: &Node, resource: &Resource) -> Result<(), String> {
    if resource.kind.is_family("npu") {
        Ok(())
    } else {
        Err(format!(
            "node `{}` uses npu mod on non-npu resource `{}` ({})",
...
fn matmul(lhs: &TensorValue, rhs: &TensorValue) -> Result<TensorValue, String> {
    if lhs.cols != rhs.rows {
        return Err(format!(
            "npu.matmul shape mismatch: lhs is {}x{}, rhs is {}x{}",
...
fn add_bias(input: &TensorValue, bias: &TensorValue) -> Result<TensorValue, String> {
    ...
    Err(format!(
        "npu.add_bias shape mismatch: input is {}x{}, bias is {}x{}",
